...
time ./build.sh $ISO 1
glance image-create --name "$GLANCE_NAME" --property hw_cpu_sockets=1 --property jenkins_build=$BUILD_NUMBER --property description=$ISO --property os_type=windows --property hw_scsi_model=virtio-scsi --property hw_disk_bus=scsi --property hw_qemu_guest_agent=yes --property os_require_quiesce=yes --disk-format raw --container-format bare --is-public true < windows-server-2012-r2.raw
